...
Import retains the complete source JSON and loads that document unchanged into Vital
WASM. It separately derives an editable PatchState projection, first attempting an
exact, round-trip-safe conversion against the pinned Vital `1.0.7` Init structure. If a
structurally valid preset uses another Vital version or material outside that subset,
the projection falls back to a warning-rich approximation. The original samples,
wavetable material, modulators, macros, filters, effects, and routes remain in the
opaque native backing state even when the projection cannot represent them.

An untouched import exports the original bytes. Supported scalar edits use Vital
controls immediately and are overlaid onto a clone of the retained document for later
structural loads and export. Structural edits replace only their owned wavetable or LFO
slot. Undo, redo, and A/B variants retain backing state independently. The persistent
top-of-workbench notice lists only active effects outside the editable controls and visible
controls affected by hidden native modulation such as macros.
...
models/styles, and any other setting changed from Init are outside the editable
projection but remain in the native backing document. Malformed JSON, missing
structure/version, invalid Base64, wrong frame sizes, non-finite samples, and
out-of-schema values are rejected before a command is committed.
...
Those features remain loaded and exportable from the backing document. Projected
patches receive a `vital-lossy` tag whenever that fallback is used.
