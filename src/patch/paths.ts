...
export const SUPPORTED_PATCH_PATHS = [
  ...
  'ampEnvelope.attackCurve',
  'ampEnvelope.decayCurve',
  'ampEnvelope.releaseCurve',
  'modEnvelope.delaySeconds',
  ...
  'modEnvelope.attackCurve',
  'modEnvelope.decayCurve',
  'modEnvelope.releaseCurve',
  'filter.enabled',
  'filter.type',
  'filter.cutoffHz',
  'filter.resonance',
...
const pathValueSchemas: Record<SupportedPatchPath, ZodTypeAny> = {
  ...
  'oscillators.0.pan': unitInterval,
  ...
  'oscillators.1.pan': unitInterval,
  ...
  'oscillators.2.pan': unitInterval,
  'ampEnvelope.delaySeconds': seconds(ENVELOPE_DELAY_MAX_SECONDS),
  ...
  'ampEnvelope.attackCurve': z.number().finite().min(-1).max(1),
  'ampEnvelope.decayCurve': z.number().finite().min(-1).max(1),
  'ampEnvelope.releaseCurve': z.number().finite().min(-1).max(1),
  'modEnvelope.delaySeconds': seconds(ENVELOPE_DELAY_MAX_SECONDS),
  ...
  'modEnvelope.attackCurve': z.number().finite().min(-1).max(1),
  'modEnvelope.decayCurve': z.number().finite().min(-1).max(1),
  'modEnvelope.releaseCurve': z.number().finite().min(-1).max(1),
  ...
  'filter.slope': z.union([z.literal(12), z.literal(24)]),
  'filter.drive': unitInterval,
  'filter.keytrack': unitInterval,
  'lfo1.enabled': z.boolean(),
  'lfo1.points': lfoPoints,
  'lfo1.rate': lfoRate,
  'lfo1.phase': unitInterval,
  'lfo1.smooth': z.boolean(),
  'lfo1.smoothing': unitInterval,
  ...
  'voice.transposeSemitones': z.number().int().min(-36).max(36),
  ...
  'effects.distortion.enabled': z.boolean(),
  'effects.distortion.type': z.enum(['soft_clip', 'hard_clip', 'sine_fold', 'bit_crush']),
  'effects.distortion.drive': unitInterval,
  'effects.distortion.mix': unitInterval,
  'effects.chorus.enabled': z.boolean(),
  'effects.chorus.voices': z.number().int().min(1).max(4),
  'effects.chorus.rate': unitInterval,
  'effects.chorus.depth': unitInterval,
  'effects.chorus.feedback': unitInterval,
  'effects.chorus.mix': unitInterval,
  ...
  'effects.reverb.predelay': seconds(REVERB_PREDELAY_MAX_SECONDS),
  'effects.reverb.lowCut': unitInterval,
  'effects.reverb.highCut': unitInterval,
}

export const PATCH_PATH_VALUE_SCHEMAS = pathValueSchemas
