...
interface CachedProjectionGeometry {
  lines: ProjectedPoint[][]
  plotBounds: WavetableWaterfallProjection['plotBounds']
}

const projectionCache = new WeakMap<WavetableState, Map<string, CachedProjectionGeometry>>()
...
export function projectWavetableWaterfall(
  ...
  const cacheKey = [viewport.width, viewport.height, sampleCount, lineCount, direction, insetRatio].join(':')
  let tableCache = projectionCache.get(wavetable)
  if (!tableCache) {
    tableCache = new Map()
    projectionCache.set(wavetable, tableCache)
  }
  let geometry = tableCache.get(cacheKey)
  ...
  if (!geometry) {
    ...
    tableCache.set(cacheKey, geometry)
  }
  const { lines, plotBounds } = geometry
