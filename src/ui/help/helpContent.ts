...
const CONTROL_COPY: Record<string, string> = {
  ...
  'Fine tune': 'Offsets pitch in cents for subtle tuning, beating, and width adjustments.',
  Frequency: 'Sets the modulation speed in cycles per second when the LFO is running freely rather than following tempo.',
  ...
  Phase: 'Chooses the point in the LFO shape where a newly played note begins. Changing it shifts the movement earlier or later without changing the shape or speed.',
  ...
  'Random phase': 'Changes where the waveform begins each time you play a note. Low values sound consistent; higher values make repeated notes feel less identical and can soften the initial click or punch.',
  ...
  'Wet send': 'Controls how much sound is sent into the reverb. At 0% the patch stays dry and close; raising it adds more room and a longer sense of space around the original sound.',
}
...
const EFFECT_COPY: Record<string, { body: string; tip: string }> = {
  ...
  reverb: {
    body: 'Places the patch in a simulated space and adds a fading tail after the sound. Wet send controls how much reverb you hear, while Size and Decay shape the space and tail.',
...
export const GUIDE_STEPS: Record<GuidePath, readonly GuideStep[]> = {
  'just-play': [
    ...
    },
    {
      ...
      tip: 'The fingerprints and attribute bars give a visual cue; your ears make the final decision.',
    },
    {
  ...
  'create-synth': [
    ...
    {
      selector: '[data-testid="effects-grid"]',
      eyebrow: 'Create synth / 05',
      title: 'Color the signal with effects',
      body: 'The Effects tab processes the combined oscillator signal. Filter the tone, add space with delay and reverb, and drag processors to change their order.',
      tip: 'Effect order matters because each processor receives the output of the one before it.',
      tab: 'modulation-effects',
    },
    {
...
function controlCopy(label: string, parent: string | null): { body: string; title: string } {
  ...
  if (label === 'Wet send') return { title: 'Reverb amount', body: CONTROL_COPY[label] }
...
export function helpArticleFor(element: Element): HelpArticle {
  ...
  if (closest(element, '.envelope-plot')) {
    return article('Amplitude envelope', 'Editable AHDSR graph', 'Shows and edits how a note rises, holds, decays, sustains, and releases over time.', 'Drag a handle on the graph, or use the five numeric controls for exact edits.')
  }
  ...
  if (closest(element, '.variant-comparison')) {
    return article('Session', 'A/B comparison', 'Keeps two patch directions available for immediate visual and audible comparison.', 'Click a variant card to select it. The active variant supplies the browser preview and export.')
  }
