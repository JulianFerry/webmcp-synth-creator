...
export function ReverbEditor({ reverb, resetKey, onChange }: { reverb: ReverbState; resetKey: number; onChange: (field: keyof ReverbState, value: unknown) => boolean }) {
  return <article className={`panel effect-editor${reverb.enabled ? '' : ' is-disabled'}`}><div className="panel-heading"><div><p className="eyebrow">Spatial tail</p><h2>Reverb</h2></div><ToggleControl checked={reverb.enabled} label="Reverb" onCommit={(v) => onChange('enabled', v)} testId="reverb-enabled" /></div><div className="control-grid effect-controls">
