...
import { ReverbVisual } from './effectVisualizations'
...
export function ReverbEditor({ reverb, resetKey, onChange }: { reverb: ReverbState; resetKey: number; onChange: (field: keyof ReverbState, value: unknown) => boolean }) {
  return <article className={`panel effect-editor reverb-editor${reverb.enabled ? '' : ' is-disabled'}`}><div className="panel-heading"><h2>Reverb</h2><ToggleControl checked={reverb.enabled} label="Reverb" onCommit={(v) => onChange('enabled', v)} testId="reverb-enabled" /></div><ReverbVisual reverb={reverb} /><div className="control-grid effect-controls">
    <ParameterSlider formatValue={(v) => `${Math.round(v * 100)}%`} id="reverb-mix" label="Mix" max={1} min={0} onCommit={(v) => onChange('mix', v)} resetKey={resetKey} step={.01} testId="reverb-mix" value={reverb.mix} />
