...
import { DelayVisual } from './effectVisualizations'
...
export function DelayEditor({ delay, resetKey, onChange }: { delay: DelayState; resetKey: number; onChange: (field: keyof DelayState, value: unknown) => boolean }) {
  return <article className={`panel effect-editor delay-editor${delay.enabled ? '' : ' is-disabled'}`}><div className="panel-heading"><h2>Delay</h2><ToggleControl checked={delay.enabled} label="Delay" onCommit={(value) => onChange('enabled', value)} testId="delay-enabled" /></div><DelayVisual delay={delay} /><div className="control-grid effect-controls">
