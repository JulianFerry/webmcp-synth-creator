...
export function DelayEditor({ delay, resetKey, onChange }: { delay: DelayState; resetKey: number; onChange: (field: keyof DelayState, value: unknown) => boolean }) {
  return <article className="panel effect-editor"><div className="panel-heading"><div><p className="eyebrow">Time-domain echo</p><h2>Delay</h2></div><ToggleControl checked={delay.enabled} label="Delay" onCommit={(value) => onChange('enabled', value)} testId="delay-enabled" /></div><div className="control-grid effect-controls">
