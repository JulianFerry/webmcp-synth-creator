...
export class VitalPresetAdapter {
  ...
  controlOperations(before: PatchState, after: PatchState): VitalControlOperation[] {
    ...
    for (const [name, value] of Object.entries(afterValues)) {
      if (!Object.is(beforeValues[name], value)) operations.push({ name, value })
    }
    return operations
  }

  importPatch(value: unknown, options?: VitalImportOptions): VitalImportResult {
