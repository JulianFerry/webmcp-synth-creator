...
Synth Creator exposes two LFOs whose routing is derived from declared state rather than
stored as editable route objects. Each LFO declares `enabled`, `target`, `scope`, and `depth`:
...
state; changing a visible Synth Creator LFO control allocates the required derived routes in
